  ...
  return (
    <div className={`bg-[#111111] border border-[#222222] rounded-xl overflow-hidden hover:border-[#0066FF]/40 transition-all cursor-pointer group ${destaque ? 'md:col-span-2' : ''}`}>
      <div className={`relative bg-[#1A1A1A] flex items-center justify-center ${destaque ? 'h-52' : 'h-36'}`}>
        ...
        <span className="absolute bottom-2 right-2 px-2 py-0.5 bg-black/80 text-white text-xs font-mono rounded">{video.duracao}</span>
      ...
      <div className="p-4">
        ...
        <h3 className="text-sm font-bold text-white group-hover:text-[#0066FF] transition-colors leading-snug line-clamp-2">{video.titulo}</h3>
...
export default function GoalPulseTV() {
  ...
  return (
    <section id="tv" className="px-6 py-12">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-xs font-semibold text-[#0066FF] uppercase tracking-widest mb-1">Multimídia</p>
            <h2 className="text-2xl font-bold text-white">📺 GoalPulse TV</h2>
          </div>
          <button className="text-sm text-[#0066FF] hover:text-white transition-colors font-medium">Ver canal →</button>
        </div>
